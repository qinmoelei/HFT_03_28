...
class DQN(object):
    def __init__(self, args):  # 定义DQN的一系列属性
        ...
        self.eval_net, self.target_net = masked_net1_with_holding_length(
            self.n_state, self.n_action, 128).to(self.device), masked_net1_with_holding_length(
    ...
    def update(
        ...
        demonstration = info["soft_q_action"]
        demonstration1 = info["previous_action_indicator"]
        ...
        loss = td_error + (KL_div+KL_div1) * self.ada
    ...
    def act(self, state, info):
        ...
        if np.random.uniform() < self.epsilon:
            actions_value = self.eval_net.forward(x, previous_action,
                                                  avaliable_action,holding_length)
    ...
    def act_test(self, state, info):
        ...
        avaliable_action = torch.unsqueeze(torch.tensor(info["holding_length"]).float(),
                                           0).to(self.device)
        holding_length = torch.unsqueeze(
            info["holding_length"], 0).to(self.device)
        actions_value = self.eval_net.forward(x, previous_action,
                                              avaliable_action,holding_length)
        action = torch.max(actions_value, 1)[1].data.cpu().numpy()
        action = action[0]
